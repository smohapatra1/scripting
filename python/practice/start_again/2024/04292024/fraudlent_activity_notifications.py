...
def activityNotifications(expenditure, d):
    ...
    s = 0 
    for i, f in zip(c, range(d, len(expenditure))):
        if d % 2 == 0:
            med = (period[d//2-1]+ period[d//2])/2
        else:
            med = period[d//2]
        if expenditure[f] >= med*2:
            s +=1
        i = bisect.bisect(period, expenditure[i])
        period.pop(i-1)
        bisect.insort(period, expenditure[f])
    return s    
